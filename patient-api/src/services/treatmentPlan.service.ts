...
class TreatmentPlanService {
  ...
  async validateTreatmentPlanOperation(treatmentId: string, userId: string) {
    ...
    if (user.role !== "doctor" && user.role !== "brand") {
      throw new Error(
        "Unauthorized: Only doctor or brand users can manage treatment plans"
      );
    }
  ...
  async createTreatmentPlan(planData: CreateTreatmentPlanData, userId: string) {
    ...
    const { treatment } = await this.validateTreatmentPlanOperation(
      planData.treatmentId,
      userId
    );
  ...
  async updateTreatmentPlan(
    planId: string,
    updateData: UpdateTreatmentPlanData,
    userId: string
  ) {
    ...
    // Validate treatment operation permission
    await this.validateTreatmentPlanOperation(
    ...
    const priceChanged =
      ...
      updateData.price !== treatmentPlan.price;
    const intervalChanged =
      updateData.billingInterval !== undefined &&
      updateData.billingInterval !== treatmentPlan.billingInterval;
  ...
  private async ensureStripePrice(
    ...
    const stripePrice = await this.stripeService.createPrice({
      ...
      currency: "usd",
      recurring: {
        interval: intervalMapping[treatmentPlan.billingInterval] as
          | "month"
          | "year",
