...
class TenantProductService {
  ...
  async validateUserPermission(
    userId: string
  ): Promise<{ user: User; clinicId: string }> {
    ...
    if (user.role !== "brand" && user.role !== "admin") {
      throw new Error(
        "Unauthorized: Only brand or admin users can manage tenant products"
      );
    }
  ...
  async validateQuestionnaires(
    ...
    if (questionnaires.length !== questionnaireIds.length) {
      ...
      const missingIds = questionnaireIds.filter(
  ...
  async validateSubscriptionLimits(
    ...
    if (!subscription) {
      throw new Error(
        "No active subscription found. Please subscribe to a plan to manage products."
      );
    }
    ...
    if (maxProducts === -1) {
      console.log(
        `✅ Subscription validation passed: ${productCount} products (limit: unlimited)`
      );
      return;
    }
    ...
    console.log(
      `✅ Subscription validation passed: ${productCount} products (limit: ${maxProducts})`
    );
  }
  ...
  async updateSelection(data: UpdateSelectionInput, userId: string) {
    ...
    const productIds = [
      ...new Set((data as any).products.map((p: any) => p.productId)),
    ] as any[];
    const questionnaireIds = [
      ...new Set(
        (data as any).products
          .map((p: any) => p.questionnaireId)
          .filter(Boolean)
      ),
    ] as any[];
    ...
    if (
      periodStart &&
      subscription.lastProductChangeAt &&
      subscription.lastProductChangeAt < periodStart
    ) {
      try {
    ...
    const existingProductIds = new Set<string>(
      existing.map((tp) => tp.productId)
    );
    const newAdds = new Set<string>(
      (productIds as string[]).filter((pid) => !existingProductIds.has(pid))
    );
    ...
    if (duplicates.length > 0) {
      throw new Error(
        `Duplicate products in selection: ${duplicates.join(", ")}`
      );
    }
    ...
    const tenantProducts = await bulkUpsertTenantProducts(
      clinicId,
      productsToUpsert
    );
    ...
    try {
      ...
      for (const tp of tenantProducts) {
        ...
        if (!stripeProductId) {
          console.log(
            `📦 Creating Stripe product for ${product.name} (${clinic?.name})`
          );
          ...
          const stripeProduct =
            await stripeService.createProduct(productParams);
        ...
        if (!tp.stripePriceId) {
          console.log(
            `💰 Creating Stripe price for ${product.name}: $${tp.price}`
          );
      ...
      if (process.env.NODE_ENV === "development") {
        ...
        console.error("⚠️ Failed to create Stripe products/prices");
      }
    ...
    try {
      const GlobalFormStructure =
        require("../models/GlobalFormStructure").default;
      ...
      for (const tp of tenantProducts) {
        ...
        const sharedQuestionnaireId =
          productQuestionnaire?.id || tp.questionnaireId || null;
        ...
        for (const structure of globalStructures) {
          ...
          if (!existingForm) {
            ...
            const protocol =
              clinic.isCustomDomain && clinic.customDomain ? "https" : "http";
            const domain =
              clinic.isCustomDomain && clinic.customDomain
                ? clinic.customDomain
                : "localhost:3000";
            ...
            console.log(
              `✅ Auto-created form for product ${product.slug} with structure ${structure.name} using ${productQuestionnaire ? "shared questionnaire" : "tenant questionnaire"}`
            );
          } else {
            ...
            if (existingForm.questionnaireId !== sharedQuestionnaireId) {
              ...
              console.log(
                `✅ Updated form for product ${product.slug} with structure ${structure.name} to use ${productQuestionnaire ? "shared questionnaire" : "tenant questionnaire"}`
              );
            }
          }
        }
      ...
      if (process.env.NODE_ENV === "development") {
        ...
        console.error("⚠️ Failed to auto-create forms");
      }
    ...
    try {
      ...
      if (process.env.NODE_ENV === "development") {
        console.warn(
          "Failed to update product change tracking fields on subscription",
          e
        );
      } else {
        console.warn(
  ...
  async updatePrice(params: {
    ...
    try {
      ...
      if (tenantProduct.clinicId !== clinicId) {
        return {
          success: false,
          error: "Tenant product does not belong to your clinic",
        };
      }
      ...
      if (!tenantProduct.product) {
        return {
          success: false,
          error: "Product not found for tenant product",
        };
      }
      ...
      const errorMessage =
        process.env.NODE_ENV === "development"
          ? error instanceof Error
            ? error.message
            : "Unknown error"
          : "Failed to update tenant product price";
