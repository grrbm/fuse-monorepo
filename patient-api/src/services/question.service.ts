...
class QuestionService {
  async validateQuestionOperation(questionnaireStepId: string, userId: string) {
    ...
    if (
      step.questionnaire?.treatment &&
      step.questionnaire.treatment.clinicId !== user.clinicId
    ) {
  ...
  async createQuestion(
    ...
    const nextQuestionOrder =
      existingQuestions.length > 0 && existingQuestions[0]?.questionOrder
        ? existingQuestions[0].questionOrder + 1
        : 1;
    ...
    if (questionData.parentQuestionId && !subQuestionOrder) {
      ...
      subQuestionOrder =
        ...
        existingSubQuestions[0]?.subQuestionOrder
    ...
    if (step) {
      ...
      if (questionnaire) {
        ...
        if (process.env.NODE_ENV === "development") {
          console.log(
            `✅ Touched questionnaire ${questionnaire.id} (${questionnaire.title}) updatedAt on question create`
          );
        }
      }
    }
    ...
    try {
      ...
      await question.update(
        {
          ...
          ...(updateData.answerType !== undefined && {
            answerType: updateData.answerType,
          }),
          ...(updateData.questionSubtype !== undefined && {
            questionSubtype: updateData.questionSubtype,
          }),
          ...
          ...(updateData.helpText !== undefined && {
            helpText: updateData.helpText,
          }),
          ...(updateData.footerNote !== undefined && {
            footerNote: updateData.footerNote,
          }),
          ...(updateData.conditionalLogic !== undefined && {
            conditionalLogic: updateData.conditionalLogic,
          }),
          ...(updateData.conditionalLevel !== undefined && {
            conditionalLevel: updateData.conditionalLevel,
          }),
          ...(updateData.subQuestionOrder !== undefined && {
            subQuestionOrder: updateData.subQuestionOrder,
          }),
        },
        { transaction }
      );
      ...
      if (step) {
        ...
        if (questionnaire) {
          ...
          if (process.env.NODE_ENV === "development") {
            console.log(
              `✅ Touched questionnaire ${questionnaire.id} (${questionnaire.title}) updatedAt`
            );
          }
        }
      }
  ...
  async deleteQuestion(questionId: string, userId: string) {
    ...
    if (step) {
      ...
      if (questionnaire) {
        ...
        if (process.env.NODE_ENV === "development") {
          console.log(
            `✅ Touched questionnaire ${questionnaire.id} (${questionnaire.title}) updatedAt on question delete`
          );
        }
      }
    }
  ...
  async saveQuestionsOrder(
    questions: Array<{ id: string; questionOrder: number }>,
    stepId: string,
    userId: string
  ) {
