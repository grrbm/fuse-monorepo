    ...
    this.intervalId = setTimeout(async () => {
        try {
            ...
            if (process.env.NODE_ENV === 'development') {
                ...
                console.error('[AUTO-APPROVE] ❌ Error processing orders');
            }
        }
        this.scheduleNextRun();
    }, interval);
...
private async processEligibleOrders(): Promise<void> {
    ...
    try {
            ...
            include: [
                {
                    model: User,
                    as: 'user',
                    attributes: ['id', 'dob'] // HIPAA: Only fetch required fields, exclude email/name
                },
                {
                    model: Treatment,
                    as: 'treatment',
        ...
        for (const order of orders) {
            try {
                ...
                if (eligible) {
                    ...
                    skippedCount++;
                    const skipReason = this.generateSkipReason(criteria);
                    if (process.env.NODE_ENV === 'development') {
                ...
                if (process.env.NODE_ENV === 'development') {
                    ...
                    console.error('[AUTO-APPROVE] ❌ Error processing order');
                }
                skippedCount++;
            }
        }
        ...
        if (process.env.NODE_ENV === 'development') {
            ...
            console.error('[AUTO-APPROVE] ❌ Error fetching orders');
        }
    }
}
...
private async evaluateOrder(order: Order): Promise<AutoApprovalCriteria> {
    ...
    const patientAgeValid = this.isPatientAgeValid(user?.dob);
...
private isPatientAgeValid(dob: string | undefined): boolean {
    ...
    try {
        ...
        return age >= this.MIN_PATIENT_AGE && age <= this.MAX_PATIENT_AGE;
    } catch (error) {
        // HIPAA: Do not log PHI (date of birth) in error messages
        console.error('[AUTO-APPROVE] Error calculating patient age');
        return false;
    }
}
...
private async autoApproveOrder(order: Order, reason: string): Promise<void> {
    ...
    if (result.success) {
        if (process.env.NODE_ENV === 'development') {
            ...
            console.log('[AUTO-APPROVE] ✅ Successfully auto-approved order');
        }
        ...
        if (process.env.NODE_ENV === 'development') {
            ...
            console.error('[AUTO-APPROVE] ❌ Failed to auto-approve order');
        }
...
async checkOrderEligibility(orderId: string): Promise<{
    ...
    const order = await Order.findByPk(orderId, {
        include: [
            { model: User, as: 'user', attributes: ['id', 'dob'] }, // HIPAA: Only fetch required fields
