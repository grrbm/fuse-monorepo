...
export class MigrationService {
  ...
  async runActiveToIsActiveMigration(): Promise<void> {
    try {
      ...
      if (process.env.NODE_ENV === "development") {
        ...
        console.error("❌ Error during active to isActive migration");
      }
  ...
  `);
      ...
      const hasActive = (columns as any[]).some(
        (col: any) => col.column_name === "active"
      );
      const hasIsActive = (columns as any[]).some(
    ...
    `);
        ...
        console.log(
          `⚠️  ${table} has both active and isActive columns - skipping migration`
        );
      } else if (!hasActive && hasIsActive) {
      ...
      if (process.env.NODE_ENV === "development") {
        ...
        console.error(`❌ Error migrating ${table}`);
      }
      throw error;
    }
  }
