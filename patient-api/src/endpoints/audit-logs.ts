...
export function registerAuditLogsEndpoints(app: Express, authenticateJWT: any, getCurrentUser: any) {
    ...
    app.get("/admin/audit-logs", authenticateJWT, async (req, res) => {
        try {
            ...
            if (startDate) {
                whereClause.createdAt = { ...whereClause.createdAt, [Op.gte]: new Date(startDate) };
            }
    ...
    app.get("/admin/audit-logs/actions", authenticateJWT, async (req, res) => {
        try {
            ...
            res.status(500).json({ success: false, message: 'Failed to fetch audit actions' });
        }
    });
    ...
    app.get("/admin/audit-logs/stats", authenticateJWT, async (req, res) => {
        try {
            ...
            const [totalLogs, last30DaysLogs, failedLogins, phiAccesses] = await Promise.all([
                AuditLog.count(),
                AuditLog.count({
                ...
                AuditLog.count({
                    where: {
                        ...
                        resourceType: { [Op.in]: [AuditResourceType.PATIENT, AuditResourceType.ORDER, AuditResourceType.PRESCRIPTION] },
                        createdAt: { [Op.gte]: thirtyDaysAgo }
                    }
                }),
            ]);
