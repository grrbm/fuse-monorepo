...
export async function extractClinicSlugFromDomain(): Promise<ClinicDomainInfo> {
  ...
  // Check if hostname is NOT a .fuse.health or .fusehealth.com subdomain and NOT localhost and NOT bare localhost
  // Skip custom domain check for localhost and localhost with port (e.g., localhost:3000)
  const isLocalhost = hostname === 'localhost' || hostname.startsWith('127.0.0.1');

  if (!hostname.endsWith('.fuse.health') && !hostname.endsWith('.fusehealth.com') && !hostname.includes('.localhost') && !isLocalhost) {
    try {
